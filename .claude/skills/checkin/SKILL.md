...
3. ゴーストファイルを除外する（後述）
4. チェックイン前にワークスペースを最新化する（「マージ処理」セクション参照）
5. Private ファイルのうちトラッキング対象を `cm add` する
6. 変更内容を分析し、論理単位にグループ分けする
7. グループごとに Conventional Commits 準拠のメッセージを生成する
8. グループごとにチェックインを実行する
9. 完了後、作成した全 changeset の一覧を報告する
...
チェックイン前にサーバーとの同期を行い、GUI マージツールを**絶対に開かない**ようにする。

### 手順

1. `cm update --dontmerge --noinput` でワークスペースを最新化する
2. update-merge が必要な場合、以下のコマンドで自動マージを試行する:

```bash
cm merge <source_spec> --merge --nointeractiveresolution --mergetype=try
```

3. 自動マージで解決できないコンフリクトがある場合:
   - コンフリクトの一覧をユーザーに報告する
   - `--keepsource`（自分の変更を優先）か `--keepdestination`（サーバー側を優先）のどちらで解決するかユーザーに確認する
   - 指示に従い再度マージを実行する:

```bash
cm merge <source_spec> --merge --nointeractiveresolution --keepsource
```

### 禁止事項

- `--nointeractiveresolution` を省略しない（GUI が起動しブロックされる）
- コンフリクトの解決方針をユーザーに確認せず勝手に決めない
