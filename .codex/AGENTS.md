...
- 英単語と英単語の区切りにはスペースを用いる。日本語の単語を列挙するときはスペースではなく・を用いる。英単語と日本語の間にはスペースを入れる
- 数字は半角英数字で記述する。日本語と数字の間にはスペースを入れない
- ファイル名やパスを記述する際はインラインコード（`example.ts`）を使用する。インラインコードブロックの前後にはスペースを入れる
